		...
		case IDOK:
			if (IsDlgButtonChecked(hSPP, IDC_RADIO1) == BST_CHECKED) {
				if (SendMessageA(GetDlgItem(hSPP, IDC_COMBO1), CB_GETCURSEL, 0, 0) == LB_ERR) {
					MessageBoxA(hSPP, "선택된 항목이 없습니다.", "ERROR", MB_OK);
					break;
				}
...
INT_PTR CALLBACK EditPlayersProc(HWND hEPSP, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
	{
		...
		case IDC_BUTTON2:
		{
			...
			if (pos == LB_ERR) {
				MessageBoxA(hEPSP, "선택된 항목이 없습니다.", "ERROR", MB_OK);
				break;
			}
...
INT_PTR CALLBACK EditPlayerProc(HWND hEPP, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
	{
		...
		case IDOK:
		{
			...
			if (nameLength > 16) {
				MessageBoxA(hEPP, "16자 이상의 이름은 사용할 수 없습니다", "ERROR", MB_OK);
				break;
			}
			else if (nameLength < 1) {
				MessageBoxA(hEPP, "이름이 입력되지 않았습니다", "ERROR", MB_OK);
				break;
			}
